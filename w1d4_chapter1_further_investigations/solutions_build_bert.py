...
from w0d2_chapter0_convolutions.solutions import Linear
from w0d3_chapter0_resnets.solutions import Sequential
from w1d4_chapter1_further_investigations.utils import print_param_count
from w1d1_chapter1_transformer_reading.solutions import Embedding, LayerNorm, GELU, Dropout
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, config: TransformerConfig):
        ...
        self.W_Q = Linear(self.hidden_size, self.num_heads*self.head_size)
        self.W_K = Linear(self.hidden_size, self.num_heads*self.head_size)
        self.W_V = Linear(self.hidden_size, self.num_heads*self.head_size)
        self.W_O = Linear(self.num_heads*self.head_size, self.hidden_size)
...
class BertCommon(nn.Module):
    ...
    def __init__(self, config: TransformerConfig):
        ...
        self.positional_encoding = Embedding(config.max_seq_len, config.hidden_size)
        self.token_type_embedding = Embedding(2, config.hidden_size)
        self.ln1 = LayerNorm(config.hidden_size, eps=config.layer_norm_epsilon)
        self.dropout = Dropout(config.dropout)
        self.layers = Sequential(*[BERTBlock(config) for _ in range(config.num_layers)])
...
def predict(model, tokenizer, text: str, k=15) -> list:
    """

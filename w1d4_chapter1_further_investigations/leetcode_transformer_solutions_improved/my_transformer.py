...
class BalancedBracketsTransformer(nn.Module):
    ...
    def forward(self, x: t.Tensor, one_zero_attention_mask: t.Tensor) -> t.Tensor:
        ...
        if x.ndim == 1:
            x = x.unsqueeze(0)
            one_zero_attention_mask = one_zero_attention_mask.unsqueeze(0)
        ...
        # Take the output corresponding to [CLS], and convert to log probabilities
        x_cls = self.log_softmax(self.final_linear(x[:, 0, :]))
        return x_cls
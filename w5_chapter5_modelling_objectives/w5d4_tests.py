...
def test_clip_model(CLIPModel):
    ...
    with t.inference_mode():
        expected = theirs(**x)
        actual = mine(**x)
    t.testing.assert_close(actual.text_embeds, expected.text_embeds, rtol=0.01, atol=1e-4)
    t.testing.assert_close(actual.image_embeds, expected.image_embeds, rtol=0.01, atol=1e-4)
...
def test_cosine_similarity(cosine_similarities):
    ...
    t.testing.assert_close(similarities, expected)
